...
class _ReservationPageState extends State<ReservationPage> {
  Future<List<Reservation>> fetchReservation(CookieRequest request) async {
    final response = await request.get('http://localhost:8000/reservasi/json/');
  ...
  Future<void> deleteData(CookieRequest request, String id) async {
    final url = Uri.parse('http://localhost:8000/reservasi/delete-reservation/$id/');
  ...
  Future<void> _showDeleteConfirmationDialog(CookieRequest request, String id) async {
    showDialog(
      context: context,
  ...
  void _showEditReservationDialog(CookieRequest request, Reservation reservation) async {
        ...
        return AlertDialog(
          ...
          content: StatefulBuilder(
            builder: (context, setState) {
              return SizedBox(
                width: MediaQuery.of(context).size.width * 0.6, // Make the dialog wider
                child: Column(
                  ...
                  children: [
                      ...
                      onTap: () async {
                        ...
                        if (pickedDate != null) {
                          setState(() {
                            ...
                            updatedTime = ''; // Reset the selected time if the date changes
                          });
                        }
                      },
                    ),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: generateFilteredTimeOptions(updatedDate).map((time) {
                    ...
                    DropdownButtonFormField<int>(
                      ...
                      items: List<int>.generate(8, (index) => index + 1).map((number) {
          ...
          actions: [
            ...
            TextButton(
              child: Text('Save', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, color: Colors.orange[800])),
              onPressed: () async {
                // Make API request to update the reservation
                final url = Uri.parse(
                    'http://localhost:8000/reservasi/edit-reservation/${reservation.fields.reservationId}/');
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Center(
              child: Column(
                ...
                children: [
                  Icon(
                    Icons.event_busy, // You can change this to any other icon you prefer
              ...
              itemBuilder: (_, index) => GestureDetector(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => DetailPage(reservation: snapshot.data![index]),
                    ),
                  );
                },
                child: Container(
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 10),
                            Text(
                              DateFormat('EEEE, d MMMM y').format(snapshot.data![index].fields.date),
                            ...
                            Row(
                              ...
                              children: [
                                ElevatedButton.icon(
                                  onPressed: () {
                                    _showEditReservationDialog(request, snapshot.data![index]);
                                  },
                                ...
                                ElevatedButton.icon(
                                  onPressed: () {
                                    _showDeleteConfirmationDialog(request,'${snapshot.data![index].fields.reservationId}');
                                  },
