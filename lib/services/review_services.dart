...
class ReviewServices {
  ...
  Future<List<Review>> getReviews({required CookieRequest request}) async {
    try {
      final response = await http
          .get(Uri.parse('$baseUrl/review/show_reviews_json_flutter/'));

      if (response.statusCode == 200) {
        final List<dynamic> reviewJson = json.decode(response.body);
        return reviewJson
            .map<Review>((dynamic json) => Review.fromJson(json))
            .toList();
      } else {
  ...
  Future<List<Review>> getRestaurantReviews({required String id}) async {
    try {
      final response = await http.get(Uri.parse(
          '$baseUrl/review/show_restaurant_reviews_json_flutter/$id/'));
  ...
  double restaurantAverageRating(
      Map<String, List<Review>>? groupedReviews, String idRestaurant) {
